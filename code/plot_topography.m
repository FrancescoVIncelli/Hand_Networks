...
function h = plot_topography(ch_list, values, make_contour, system, ...
    ...
    plot(head_x, head_y, 'Color', [0.2 0.2 0.2], 'LineWidth',4);
    hold on;
    ...
    plot(nose_x, nose_y, 'Color', [0.2 0.2 0.2], 'LineWidth',4);
    hold on;
    ...
    plot(ear_x_right+head_rho+offset, ear_y_right, 'Color', [0.2 0.2 0.2], 'LineWidth',4); hold on;
    %%%% plot(ear_x_left-head_rho-offset, ear_y_left, 'Color', 'k', 'LineWidth',4); hold on;
    plot(ear_x_left-head_rho-offset, ear_y_left, 'Color', [0.2 0.2 0.2], 'LineWidth',4); hold on;
    ...
    if plot_channels, he = scatter(ch_x, ch_y, 60,'MarkerEdgeColor', [0.3,0.3,0.3],'MarkerFaceColor','w','LineWidth',1.2); end
    %%%% if plot_clabels, text(ch_x, ch_y, ch_list); end
    if plot_clabels, text(ch_x, (ch_y+0.02), ch_list); end
    ...
    %%%% cbar=colorbar;   % Feel free to modify caxis after calling the function
    %%%% set(get(cbar,'label'),'string','Nodes Local Efficiency');
    axis square;
    axis off;
    hold off;
    h = gcf;
